...
package body Afficher is
   ...
   begin
      ...
      E := Next_Event ;
      while not (E.Trouve and E.Etype=Button_Press and (Sqrt(Float(((E.X-X_1)**2)+((E.Y-Y)**2)))<Float(R) or Sqrt(Float(((E.X-X_2)**2)+((E.Y-Y)**2)))<Float(R))) loop
	 E := Next_Event ;
      end loop ;
      
      return Sqrt(Float(((E.X-X_2)**2)+((E.Y-Y)**2)))<Float(R);
   end Choix_Binaire;
